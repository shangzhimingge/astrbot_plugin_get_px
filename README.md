...
管理中心的“会话策略”页可按群 ID 或用户 ID 管理两份独立列表。内置词开启时使用内置词与内容安全页全局列表并忽略独立列表；关闭时仅使用当前会话独立列表。批量应用接口 `POST content-safety/policies/apply-field` 可将单个列表复制到全部群聊、私聊或全部现有策略；旧配置自动补空列表，不迁移全局数据。
