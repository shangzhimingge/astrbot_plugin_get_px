...
| 内容安全 | 普通分级限制与内置安全词可按群或私聊用户独立开关；自定义安全词、作品 ID 黑名单全局生效 |
| 每日签到 | H 纸张画册卡片、竖向随机背景、金币、好感度、连签、商店与主题 |
| 管理中心 | 群排行与趋势、成员数值、安全词与黑名单、签到备份 |
| 稳定性 | 0–7 个自然日去重、发送失败重试、临时文件自动清理 |

管理中心的“会话策略”页可在“群聊策略”和“私聊策略”间切换，并按群 ID 或用户 ID 分别管理“仅普通分级”和“启用内置安全词”。未配置或删除后的会话保持两项开启；关闭内置词不会影响全局自定义安全词，作品 ID 黑名单也始终全局生效。AstrBot 插件配置页提供同样的两份可添加、删除模板列表；当前配置文件是策略数据源，升级时仅一次性导入旧 SQLite 群策略。
