...
管理中心的“群策略配置”页提供两个互相独立的群级开关：“仅普通分级”和“启用内置安全词”。旧数据库、未配置群和私聊都保持两项开启；关闭内置词不会影响全局自定义安全词，作品 ID 黑名单也始终全局生效。群策略可在 AstrBot 插件配置页和插件管理中心中管理；两处共用同一份持久化配置。删除群后，该群恢复两项均开启的默认严格策略；升级时会一次性导入旧 SQLite 策略。
