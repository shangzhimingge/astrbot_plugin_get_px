...
| 字段 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `checkin_omnidraw_quota_cost` | `int` | `150` | 签到商店购买一次万象画卷生图额度所需金币，取值 0–10000；设为 0 表示免费 |
| `checkin_omnidraw_quota_pack` | `int` | `5` | 每次购买到账的生图张数，取值 1–50；额度计入万象画卷当日生图限额，跨天清零 |

两个配置仅在实际安装万象画卷插件且其启用每日生图限制时生效；未安装或未启用时，生图额度商品不出现在商店目录中。
