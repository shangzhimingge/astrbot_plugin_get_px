...
普通分级限制和内置安全词通过 `_conf_schema.json` 中的两份模板列表持久化，也可在管理中心按群或私聊用户编辑：

| 策略 | 未配置/已删除会话 | 关闭后的边界 |
| --- | --- | --- | --- |
| 仅普通分级 | 开启 | Lolicon 请求普通/R18 混合候选，Pixiv 回退不再剔除受限分级 |
| 启用内置安全词 | 开启 | 仅跳过内置词；自定义安全词仍全局生效 |

作品 ID 黑名单始终全局生效。群策略保存在 `group_content_safety_policies`，私聊策略保存在 `private_content_safety_policies`；配置文件是当前数据源。旧 `checkin.sqlite3` 群策略只在升级时导入一次，签到 JSON 导入不重置会话策略。
