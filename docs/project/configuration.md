...
| 字段 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `checkin_omnidraw_link_enabled` | `bool` | `false` | 万象画卷联动总开关；开启后才会检测已安装的万象画卷插件，并启用商店生图额度、签到发放额度、`/签到状态` 额度展示等全部联动功能。未开启时所有联动一律不生效 |
| `checkin_omnidraw_quota_cost` | `int` | `75` | 签到商店购买生图额度的单张价格，取值 0–1000；实际花费 = 单价 × 张数；设为 0 表示免费 |

以上配置仅在 `checkin_omnidraw_link_enabled` 开启、且实际安装万象画卷插件并启用其每日生图限制时生效；联动开关关闭或对方未安装/未启用时，生图额度商品不出现在商店目录中。购买张数由指令参数指定（`签到商店 生图 <张数>`，范围 1–50），额度计入万象画卷当日生图限额，跨天清零。
